...
// ── Auth helper: verifica access_level >= 1 en BD ──────────

async function requireProAccess(request, env) {
  const user = await authenticate(request, env);
  if (!user) return null;
  // Verificar en BD (nunca solo confiar en el JWT)
...
// ── Contexto ligero — solo para Haiku (~800 tokens) ────────
...
// ── Contexto completo — para Sonnet (~2500 tokens) ─────────
...
async function buildUserContext(userId, env) {
  ...
  const [user, todayRows, last7Days, last30Stats, topFoods, weightHistory, calibration] =
    await Promise.all([
      env.DB.prepare(
      ...
      env.DB.prepare(
        'SELECT global_bias, confidence, data_points FROM user_calibration WHERE user_id = ?'
      ).bind(userId).first(),
    ]);
...
=== CALIBRACIÓN IA ===
${calibration ? `${calibration.data_points} correcciones | Confianza: ${Math.round(calibration.confidence*100)}%` : 'Sin datos de calibración'}`;
}
...
function detectQueryComplexity(message) {
  ...
  const complex = [
    ...
    /qu[eé] ha pasado esta (semana|semana|mes)/i,
  ];
...
async function generateConversationTitle(convId, userMessage, apiKey, db) {
  try {
...
export async function handleAssistant(request, env, path, ctx) {
  ...
  if (path === '/api/assistant/chat' && request.method === 'POST') {
    ...
    const { message, conversation_id, is_intro } = body;

    if (!message?.trim() || message.length > 1000) {
      return errorResponse('Mensaje inválido (máx 1000 caracteres)', 400);
    }

    // Verificar límite diario (los intros no cuentan)
    const today = new Date().toLocaleDateString('en-CA');
    if (!is_intro) {
    ...
    const maxTokens   = complexity === 'sonnet' ? 1000 : 500;
    ...
    // Historial de la conversación (no para intros)
    const history = (!is_intro && convId)
      ? await env.DB.prepare(
          'SELECT role, content FROM assistant_messages WHERE conversation_id = ? ORDER BY created_at ASC LIMIT 10'
        ).bind(convId).all()
      : { results: [] };

    // Construir contexto según complejidad
    const userContext = complexity === 'sonnet'
    ...
    // Los intros no se guardan en historial ni consumen límite
    if (!is_intro && convId) {
      const limit = DAILY_LIMITS[user.access_level] ?? 20;
      const usage = await env.DB.prepare(
        'SELECT messages FROM assistant_usage WHERE user_id = ? AND date = ?'
      ...
      if (isNewConv && ctx) {
        ctx.waitUntil(generateConversationTitle(convId, message, env.ANTHROPIC_API_KEY, env.DB));
      }
  ...
  if (convMatch && request.method === 'GET') {
    ...
    const msgs = await env.DB.prepare(
      'SELECT role, content, model_used, created_at FROM assistant_messages WHERE conversation_id = ? ORDER BY created_at ASC'
    ).bind(convId).all();
