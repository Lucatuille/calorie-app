...
export default function AdvancedAnalytics({ isOpen, onClose, userTarget }) {
  ...
  // Projection chart data: historical weight points + future projections
  const projChartData = (() => {
    ...
    if (!weightPts.length || !data.projection?.projection_30d) return weightPts;

    const today = new Date();
    const fmt   = n => new Date(today.getTime() + n * 86400000).toLocaleDateString('es', { day: 'numeric', month: 'short' });
    return [
      ...weightPts,
      { date: 'Hoy',    actual: data.weight.current, projected: data.weight.current },
      { date: fmt(30),  projected: data.projection.projection_30d },
      { date: fmt(60),  projected: data.projection.projection_60d },
      { date: fmt(90),  projected: data.projection.projection_90d },
    ];
  })();
                ...
                {data.weight?.current == null ? (
                  ...
                  <>
                    <div style={{ display: 'flex', gap: 24, marginBottom: 18, flexWrap: 'wrap' }}>
                      ...
                      {data.weight.trend_per_week != null && (
                        <div>
                          <p style={{ fontSize: 11, color: 'var(--text-3)', marginBottom: 2 }}>Tasa semanal</p>
                          <p style={{
                            fontFamily: 'Instrument Serif', fontSize: 30, lineHeight: 1,
                            color: data.weight.trend_per_week < 0 ? 'var(--accent)' : 'var(--accent-2)',
                          }}>
                            {data.weight.trend_per_week > 0 ? '+' : ''}{data.weight.trend_per_week} kg/sem
                          </p>
                        </div>
                      )}
                    </div>
                    ...
                    {projChartData.length > 1 && (
                      <ResponsiveContainer width="100%" height={180}>
                        <ComposedChart data={projChartData} margin={{ top: 5, right: 10, bottom: 5, left: -10 }}>
                          ...
                          <Tooltip
                            formatter={(v, name) => [`${v} kg`, name === 'actual' ? 'Real' : 'Proyectado']}
                            contentStyle={{ background: 'var(--surface)', border: '1px solid var(--border)', borderRadius: 8, fontSize: 12 }}
                          />
                          <Line
                          ...
                          <Line
                            ...
                            connectNulls={false} strokeOpacity={0.55}
                          />
                        </ComposedChart>
                      </ResponsiveContainer>
                    )}

                    {data.projection?.projection_30d && (
                      <div style={{ display: 'flex', gap: 8, marginTop: 16 }}>
                        {[
                          { label: 'En 30 días', val: data.projection.projection_30d },
                          { label: 'En 60 días', val: data.projection.projection_60d },
                          { label: 'En 90 días', val: data.projection.projection_90d },
                        ].map(p => (
                          <div key={p.label} style={{ flex: 1, background: 'var(--bg)', borderRadius: 10, padding: '10px 8px', textAlign: 'center' }}>
                            <p style={{ fontSize: 10, color: 'var(--text-3)', marginBottom: 4 }}>{p.label}</p>
                            <p style={{ fontWeight: 700, fontSize: 15 }}>{p.val} kg</p>
                          </div>
                        ))}
                      </div>
                    )}

                    {data.projection?.days_to_goal && (
                      <div style={{
                        ...
                        <p style={{ fontSize: 13, color: 'var(--accent)', fontWeight: 600 }}>
                          🎯 A tu objetivo ({data.projection.goal_weight} kg) le quedan ~{data.projection.days_to_goal} días
                        </p>
                        <p style={{ fontSize: 12, color: 'var(--text-3)', marginTop: 2 }}>si mantienes este ritmo</p>
                      </div>
                    )}
